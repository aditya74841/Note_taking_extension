...
export default function App() {
  ...
  // Tracks when the user has explicitly opened a note (Main URL, list click)
  // that differs from the current tab. Prevents tab events from resetting the editor target.
  const manualEditOverrideRef = useRef(false);
    ...
    async (isInitial = false) => {
      ...
      try {
        ...
        if (!context) {
          ...
          manualEditOverrideRef.current = false;
          if (!isSavePendingRef.current) setEditorContent('');
          setWebsiteNotes([]);
          // Default to 'all' nav tab on non-web page so saved notes are immediately visible
          setActiveNav('all');
          return;
        }

        // Clear manual override if the tab URL actually changed (user navigated)
        if (prevContext && prevContext.urlKey !== context.urlKey) {
          manualEditOverrideRef.current = false;
        }
        ...
        // If user explicitly opened a note (Main URL, list click), don't reset editor target
        // Only refresh lists and main note status
        if (manualEditOverrideRef.current && !isInitial) {
  ...
  // Persist current note changes safely (Never auto-delete notes on empty input or tab switches)
  const persistEditorNote = useCallback(
    async (nextContent: string, targetKey: string, targetTitle: string, targetFullUrl: string) => {
      ...
      const titleToSave = targetTitle || targetFullUrl;

      // Soft-delete if user erased all plain text from an existing note
      if (plainText.length === 0) {
        const existing = await getNote(targetKey);
        if (existing) {
      ...
      // Sync title changes to pinnedMap if this note is pinned
      const currentPin = pinnedMapRef.current[domain];
      if (currentPin && currentPin.urlKey === targetKey) {
        if (currentPin.title !== titleToSave) {
          const nextMap = {
            ...pinnedMapRef.current,
            [domain]: { ...currentPin, title: titleToSave },
          };
          pinnedMapRef.current = nextMap;
          setPinnedMap(nextMap);
          if (typeof browser !== 'undefined' && browser.storage?.local) {
  ...
  const openInEditor = async (noteKey: string, defaultTitle: string, fullUrl: string) => {
    ...
    // Update refs immediately so any in-flight auto-save uses the correct target
    editorTargetUrlKeyRef.current = noteKey;
    editorTargetFullUrlRef.current = fullUrl;
    ...
    editorTargetTitleRef.current = resolvedTitle;
    setEditorContent(existingNote?.content ?? '');
    setLastSavedTime(existingNote?.updatedAt ?? null);
    setSaveStatus('idle');
    setIsPreviewMode(false);

    // Mark manual override so tab events don't reset the editor target
    const context = contextRef.current;
    if (context && noteKey !== context.urlKey) {
      manualEditOverrideRef.current = true;
    } else {
      manualEditOverrideRef.current = false;
    }
